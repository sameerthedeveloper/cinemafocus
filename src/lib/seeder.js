...
// Helper to upload local public assets to Supabase Storage
const uploadLocalAsset = async (path) => {
    ...
    try {
        ...
        const filePath = `seeded_assets/${Date.now()}_${filename}`;
        ...
        console.warn(`Failed to upload asset to Supabase: ${path}`, error);
...
export const seedDatabase = async () => {
    ...
    try {
        await batch.commit();
        console.log("Database seeded & Assets migrated to Supabase successfully!");
        alert("Database seeded & Assets migrated to Supabase Storage successfully!");
    } catch (error) {
