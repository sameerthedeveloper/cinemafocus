...
const fetchSiteSetting = async (supabase, settingId) => {
    // Try site_settings first
    let result = await supabase.from("site_settings").select("data").eq("id", settingId).single();
    
    // If table doesn't exist (PGRST205), try site_content
    if (result.error && result.error.code === 'PGRST205') {
        result = await supabase.from("site_content").select("data").eq("id", settingId).single();
    }
